  ...
  <p>这是一个 Tampermonkey 用户脚本。它会先用本地白名单、黑名单、标题关键词和正则规则判断 B 站首页推荐；只有本地规则无法确定且没有可用缓存的视频，才会批量交给用户配置的 AI 接口进行语义判断。AI 命中只隐藏当前视频，不会自动永久拉黑整个 UP 主；同一 UP 主多次高置信度命中后，脚本会提示用户自行确认是否拉黑。</p>

  <h2>当前版本范围</h2>
  <ul>
    <li>只处理 <code>https://www.bilibili.com/</code> 首页。</li>
    <li>支持首页首屏、无限滚动和“换一换”产生的新卡片。</li>
    <li>支持在首页视频卡片上右键添加不喜欢样本，并自动生成 AI 偏好画像；兼容 BV、AV 链接和特殊悬停预览卡片。</li>
    <li>支持右键手动拉黑 UP 主或加入 UP 主白名单。</li>
    <li>支持本地标题黑白名单，规则可以是普通关键词或 <code>/正则表达式/</code>。</li>
    <li>不处理搜索页、排行榜、视频页、B站 App 或观看历史。</li>
  ...
  <ol>
    ...
    <li>打开或刷新 B 站首页，右下角会显示“AI 过滤”按钮。</li>
  </ol>

  <h2>配置</h2>
  <ol>
    <li>点击页面右下角“AI 过滤”，或使用 Tampermonkey 菜单中的“打开 B站首页 AI 过滤设置”。</li>
  ...
  <h2>本地规则与 AI 判断顺序</h2>
  <ol>
    <li>手动标记过“不喜欢”的视频 ID（BV 或 AV）：隐藏当前视频。</li>
    <li>UP 主白名单或标题白名单命中：保持显示，不再调用 AI。</li>
    <li>UP 主黑名单或标题黑名单命中：直接隐藏，不再调用 AI。</li>
  ...
  <h2>右键菜单与主动学习</h2>
  <ol>
    <li>在 B 站首页的视频卡片上点击右键。</li>
    <li>选择“不喜欢此视频 · 隐藏并让 AI 学习”，只处理当前视频 ID。</li>
    <li>也可以选择“手动拉黑该 UP 主”，隐藏该 UP 主之后的首页推荐。</li>
    <li>对希望始终显示的作者选择“始终显示该 UP 主”，加入 UP 主白名单。</li>
    <li>该视频的 BV 或 AV 号会立即进入本地不喜欢样本并从首页隐藏。</li>
    <li>配置了 API Key 时，AI 会自动分析标题和 UP 主名称，提炼内容特征并更新偏好画像。</li>
    <li>后续首页推荐会同时参考手写过滤描述、偏好画像和近期不喜欢样本。</li>
  </ol>
  <p>AI 高置信度命中也只隐藏当前视频。同一 UP 主累计至少 3 个不同视频命中后，设置面板会显示“拉黑”或“忽略”建议，只有用户点击“拉黑”才会写入永久 UP 主黑名单。按住 <kbd>Shift</kbd> 再点击右键，可以使用浏览器原生右键菜单。未配置 API Key 时样本仍会保存，保存有效配置后会自动补充分析。</p>
  ...
  <ul>
    ...
    <li>暂停期间，本地标题规则、UP 主黑白名单、视频不喜欢样本和已有 AI 缓存继续生效；页面滚动和新推荐不会自动产生 API token 消耗。</li>
  ...
  <h2>Token 节省机制</h2>
  <ul>
    <li><strong>本地优先：</strong>白名单、UP 主黑名单、视频不喜欢样本、标题关键词和正则规则都在浏览器内判断，命中时完全不调用 AI。</li>
    <li><strong>持久缓存：</strong>AI 对一个视频 ID（BV 或 AV）的匹配和不匹配结果都会缓存在 Tampermonkey 本地存储中。刷新页面、无限滚动或卡片重新出现时可直接复用。</li>
    ...
    <li><strong>缩短输入：</strong>只发送最近 3 个学习样本；标题最多 160 字、UP 主名称最多 60 字，过滤描述和学习画像各最多 400 字；历史样本不再重复发送 UP 主名称。</li>
  ...
  <h2>黑名单与备份</h2>
  <ul>
    <li>黑名单使用 UP 主 UID 作为唯一键，保存在 Tampermonkey 本地存储中。</li>
    <li>UP 主白名单同样按 UID 保存，并优先于自动判断和普通黑名单。</li>
    <li>手动不喜欢样本使用规范化的视频 ID（BV 或 AV）作为唯一键，与 UP 主黑名单分开保存。</li>
    <li>可在设置面板中删除误判的黑名单、白名单和未采用的 AI 候选规则。</li>
    <li>“导出”会保存过滤描述、非敏感设置、UP 主黑白名单、标题黑白名单、不喜欢样本和学习画像。</li>
    <li>导出文件不会包含 API Key 或 AI 判断缓存。</li>
    <li>“导入”会按 UID 和视频 ID 合并数据，并保留当前设备的 API Key；v0.1.0、v0.2.0 和 v0.3.x 备份均可导入。</li>
  </ul>

  <h2>隐私说明</h2>
  <p>只有本地规则和缓存无法判断时，AI 判断才会把过滤描述、偏好画像、近期不喜欢样本的视频标题，以及待判断视频的标题和页面显示的 UP 主名称发送给所选 API 服务商。主动学习会发送当前手动不喜欢样本的标题和 UP 主名称。脚本不会主动发送 B 站 Cookie、账号资料或页面正文。API Key 仅存放在当前 Tampermonkey 脚本存储中，但浏览器本地存储不等同于系统级密码保险箱。</p>
