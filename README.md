<!-- AI-Model-Signature: gpt-5.6-sol | 2026-08-22 | 添加 v0.7.5 B 站首页推广视频屏蔽说明 -->
  ...
  <!-- AI-Model-Signature: grok-4.5 | 2026-07-19 | 更新 v0.7.3 自动监视默认关闭与风险确认说明 -->
  ...
  <ol>
    ...
    <li>如需语义过滤，勾选「启用自动 AI 监视」并在风险提示中确认；默认关闭时仅本地规则生效。</li>
    <li>按需填写本地标题黑名单和白名单，每行一条关键词或 <code>/正则表达式/</code>。</li>
    <li>点击“保存”。开启自动监视后，脚本才会把本地无法判断的推荐交给 AI。</li>
  </ol>
  ...
  <ol>
    ...
    <li>如果标记错了，可在「AI 主动学习」列表点击“撤销”，或对已标记视频再次右键撤销。</li>
  </ol>
  <p>撤销会立即重新显示当前视频、删除该视频的 AI 缓存，并只用剩余已分析样本在本地重建偏好画像，不会为重建额外消耗 token。AI 高置信度命中也只隐藏当前视频。同一创作者累计至少 3 个不同视频命中后，设置面板会显示“拉黑”或“忽略”建议，只有用户点击“拉黑”才会写入永久创作者黑名单。按住 <kbd>Shift</kbd> 再点击右键，可以使用浏览器原生右键菜单。未配置 API Key 时样本仍会保存，保存有效配置后会自动补充分析。</p>
  ...
  <h2>自动 AI 监视（默认关闭）</h2>
  <ul>
    <li>设置面板中的「启用自动 AI 监视」控制是否自动把本地无法判断的视频发给 AI；<strong>默认关闭</strong>。</li>
    <li>首次勾选、保存开启，或点击右下角 <code>▶</code> 开启时，会弹出隐私风险确认：说明可能发送标题与创作者显示名。</li>
    <li>浮动按钮 <code>⏸</code>/<code>▶</code> 与设置开关同步，状态会写入 Tampermonkey 本地存储。</li>
    <li>关闭期间，本地标题规则、创作者黑白名单、视频不喜欢样本和已有 AI 缓存继续生效，不会自动消耗 API token。</li>
    <li>关闭期间右键「不喜欢」仍可立即隐藏；配置了 API Key 时仍可按用户操作学习，但不会顺带打开自动监视。</li>
    <li>“测试连接”和“AI 提炼候选规则”属于用户主动操作，不受自动监视开关影响。</li>
    <li>关闭前已经发出的请求返回后会被忽略，避免旧结果改变页面。</li>
  </ul>
  ...
  <ul>
    ...
    <li><strong>默认关闭自动监视：</strong>设置开关或浮动按钮可开启/关闭首页自动判断；关闭时本地规则与缓存仍正常工作。</li>
  ...
  <ul>
    ...
    <li>可在设置面板中撤销不喜欢样本，也可删除误判的创作者黑名单和白名单。</li>
  ...
  <h2>简要更新日志</h2>
  <h3>v0.7.4（2026-08-20）</h3>
  <ul>
    <li>「AI 主动学习」列表为每个不喜欢样本增加“撤销”，并显示全部最多 50 个样本。</li>
    <li>对已标记视频再次右键，可直接撤销并重新显示。</li>
    <li>撤销时同步删除该视频缓存、废弃过期首页判断和学习请求，并根据剩余样本在本地重建偏好画像。</li>
    <li>修复 GitHub Actions 中隐私确认框导致两个首页测试超时的问题；当前包含 34 项核心测试及 6 组浏览器集成测试。</li>
  </ul>
  <h3>v0.7.3（2026-07-19）</h3>
  <ul>
    <li>设置中新增「启用自动 AI 监视」开关，默认关闭。</li>
    <li>开启前会提示：本地无法判断时才会把标题与创作者显示名发给所选 AI 服务商。</li>
    <li>右下角 ▶/⏸ 与设置开关同步；关闭时本地规则与缓存仍生效。</li>
  </ul>
